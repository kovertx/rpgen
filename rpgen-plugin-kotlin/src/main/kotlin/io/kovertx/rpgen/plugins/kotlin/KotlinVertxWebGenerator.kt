...
object KotlinVertxWebGenerator : KotlinGeneratorBase<Unit>() {
    override val id: String = "vertx-web"
        ...
        ctx.schemas.https.forEach { http ->
            ...
            ctx.output.writeFile("${pkgPath}/Generated${http.name}Routes.kt") {
                smartly {
                    ...
                    ln("import io.vertx.ext.web.Route")
                    ...
                    printControllerInterface(http)
                    printRoutesHelper(http)
                }
            }
        }
    }
    ...
    private fun SmartWriter.printControllerInterface(http: RpHttp) = ln()
        .blockComment {
            ln("Generated interface defining methods needed to implement ${http.name} HTTP API.")
        }
        .p("interface I${http.name}Controller ").curly {
            ...
            ln().blockComment {
                ln("Injector called before all other handlers when defining a route (once for each")
                ln("route)")
            }
            ln("fun beforeAllHandlers(route: Route) = route")

            ln().blockComment {
                ln("Injector called after all other handlers when defining a route (once for each")
                ln("route)")
            }
            ln("fun afterAllHandlers(route: Route) = route")
            ...
            ln("fun beforeAnyAuthHandler(route: Route) = route")
            ...
            ln("fun afterAnyAuthHandler(route: Route) = route")

            ln().blockComment {
                ln("Called once before all routes are mapped to the controller")
            }
            p("fun beforeAllRoutes(router: Router) ").curly {
                ln("/* default no-op */")
            }

            ln().blockComment {
                ln("Called once after all routes are mapped to the controller")
            }
            p("fun afterAllRoutes(router: Router) ").curly {
                ln("/* default no-op */")
            }
        }
    ...
    private fun SmartWriter.printRoutesHelper(http: RpHttp) = ln()
        .blockComment {
            ln("Adds handlers to the Router mapping endpoints to the given controller")
        }
        .p("fun Router.addRoutes(controller: I${http.name}Controller) ").curly {
            // define auth handlers
            http.authSchemes.forEach { authScheme ->
                ln("val auth${authScheme.name}Handler = controller.create${authScheme.name}AuthHandler()")
            }

            ln()
            ln("controller.beforeAllRoutes(this)")

            http.flatEndpoints.forEach { endpoint -> printEndpointHandler(endpoint) }

            ln()
            ln("controller.afterAllRoutes(this)")
        }

    private fun SmartWriter.printEndpointHandler(endpoint: RpHttpEndpoint) {
        val verb = endpoint.verb.name.toLowerCase()
        val authName = endpoint.authConstraints?.name

        ln().p(verb).p("(\"").p(endpoint.path.toPathStr()).p("\")")
        indent {
            ln(".let { route -> controller.beforeAllHandlers(route) }")
            if (endpoint.bodyType != null) ln(".consumes(\"application/json\")")
            if (authName != null) {
                ln(".let { route -> controller.beforeAnyAuthHandler(route) }")
                ln(".handler(auth${authName}Handler)")
                ln(".let { route -> controller.afterAnyAuthHandler(route) }")
            }
            ln(".let { route -> controller.afterAllHandlers(route) }")
            ln(".handler { ctx ->")
            indent {
                ...
                endpoint.path.filterIsInstance<RpHttpPathParam>().forEach { param ->
                    ...
                    else p("controller.decode").printTypeRef(param.type)
                ...
                endpoint.queryParams.forEach { param ->
                    ...
                    indent {
                        if (param.type.type != "str") ln(".map { controller.decode${param.type.type}(it) }")
                        ln(".firstOrNull()")
                    }
                ...
                p("controller.").p(endpoint.name).p("(")
